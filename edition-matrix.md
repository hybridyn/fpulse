...
| 37 node types across 6 categories | ✅ | ✅ |
| 43 OSS connectors (4 database dialects via Database Source/Sink + 2 bulk-load dialects (Postgres COPY, Snowflake PUT+COPY) + 37 SaaS manifests). Cert matrix at `/api/connectors/cert-matrix` reports live depth scores: 1 beta (Salesforce v2, depth-3), 36 v1 manifests still on the migration path. v1→v2 + fixture coverage uplift is the post-1.0 connector roadmap. | ✅ | ✅ |
| Expression engine (`$json`, `$now`, `$('Node')`) | ✅ | ✅ |
| SQL Transform (DuckDB-backed) | ✅ | ✅ |
| Templates (20 OSS) | ✅ | ✅ + private marketplace |
| 20 OSS templates (simple ETL, dedup, aggregation, data quality) | ✅ | ✅ |
...
| Python Transform (Docker-isolated, secrets injected via Vault) | — | ✅ |
...
The agentic Copilot and its 16 tools all ship in OSS. Plus adds organizational guardrails.

| Capability | F-Pulse | F-Pulse+ |
|---|:-:|:-:|
| Copilot dock (resizable, page-aware, slash commands) | ✅ | ✅ |
| 20 tools (15 READ + 4 SAFE_WRITE + 1 HIGH_IMPACT_WRITE) — full list in `docs/product_facts/10_ai_copilot.md` | ✅ | ✅ |
| Fast-lane router (rule-based; sub-1 s answers for the 14 most-common questions) | ✅ | ✅ |
| Three-layer chat knowledge: session context (Layer 1) + product knowledge RAG (Layer 2) + 20 tools (Layer 3) | ✅ | ✅ |
