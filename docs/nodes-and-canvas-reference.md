...
**Companion file:** [`docs/FRONTEND_FIELD_AUDIT_RAW.md`](FRONTEND_FIELD_AUDIT_RAW.md) — full per-node tables, every field with type / default / placeholder / required / show-when conditions. 60 nodes covered (53 hardcoded configs + 6 Data Wrangler sub-steps + 1 universal Settings tab). Use that file as the source of truth when validating individual nodes.
...
The full per-node tables (every field, every default, every show-when condition) live in **[FRONTEND_FIELD_AUDIT_RAW.md](FRONTEND_FIELD_AUDIT_RAW.md)**. Structure:
...
| Concern | File |
|---|---|
| Frontend per-node field audit (raw tables, 60 nodes) | `docs/FRONTEND_FIELD_AUDIT_RAW.md` |
