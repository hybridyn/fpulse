...
- **27 pipeline templates** (Medallion ETL, CDC, Data Quality, etc.)
...
- **Single-operator install, instance roles** — OSS Free runs as a single admin account (you create it on first launch — nothing is auto-provisioned) scoped to a single default workspace; there is no multi-user role model. The role vocabulary is `viewer` / `developer` / `admin` / `super_admin`. **Multi-user team workspaces, per-workspace RBAC, cross-workspace correlation, SSO, and RBAC-aware approval chains are [F-Pulse+](../edition-matrix.md) features.**
