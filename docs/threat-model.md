...
| Smuggling code via Python transform | `python_transform` node runs in-process with full process privileges | Same as above. Documented in `nodes/python_transform.py` docstring. |
...
| In-process sandboxing of Python transforms | Would require `seccomp` / `nsjail` — explicitly an F-Pulse+ feature when introduced. |
