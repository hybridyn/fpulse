...
{"kind": "dismiss",
 ...
 "reason": "Sales Pipeline reads leads-1000 for daily reporting; Siva reads it for ad-hoc analysis. Different SLAs, intentional."}
```
...
2. **`Sales Pipeline` + `Siva`** — both read `d9880bfabd79.leads-1000` (the MSSQL leads table)
3. **`First Pipeline (copy)` + `Siva`** — both read `products-100.csv`

These are real overlaps in the user's actual workspace. Detector produced them deterministically from the workflow store contents. The pipeline `Siva` ingests 2 sources that are already used by other pipelines — exactly the kind of "I built this without realising it overlapped" pattern the Steward is designed to catch.
