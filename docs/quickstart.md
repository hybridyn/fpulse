...
# Build the UI first — F-Pulse serves its own frontend and the built
# bundle isn't committed. Skip this and the API runs but / is blank.
...
> **On a corporate / office network?** If `pip install` or the first
> `docker compose up` build fails with `SSL: CERTIFICATE_VERIFY_FAILED …
> self-signed certificate in certificate chain`, that's your company's
> TLS-inspection proxy — it affects every Python install, not just F-Pulse.
> Fix it by trusting your org's root CA (`pip config set global.cert
> <corp-ca.pem>` plus `REQUESTS_CA_BUNDLE`), or as a quick unblock add
> `--trusted-host pypi.org --trusted-host files.pythonhosted.org` to the pip
> command. Full note in the README Quick-start.
...
Open the **F-Pulse Copilot** from the bottom-right corner. The agent has 21 read tools (part of 26 total) to summarize your workspace, inspect connections, query metrics, and explain failures. To enable it:
