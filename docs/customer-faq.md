...
**No-training guarantee:** we use providers configured for zero-retention API tiers. Per-request `disable_training` / `store: false` flags set where supported.
...
- **At rest:** AES-256-GCM with per-workspace DEK + vendor KEK (F-Pulse+).
- **In transit:** TLS 1.3 only at the reverse-proxy boundary.
...
| Standard | Status |
|---|---|
| **GDPR** | PII redaction layer (configurable per workspace) + Vault credential isolation + audit trail. Article 30 records of processing supported via trace export. |
| **SOC2 Type II** | Readiness audit Q3 2026. Evidence base: approval snapshots + agent traces + existing audit log. |
| **DORA (EU operational resilience)** | Approval snapshot artifacts + replay-safe traces + tier 5 RBAC + sandbox isolation = the evidence shape DORA auditors look for. Formal certification post-Q3 2026. |
| **HIPAA** | Not currently certified. On-prem Ollama option supports the deployment shape PHI handlers require; full BAA workflow Q4 2026. |
| **India compliance** | LUT filing for FY 2026-27. Aadhaar + India PII patterns in default redaction list. |
| **PCI** | Credit-card patterns in default redaction denylist. Not a card processor; out of scope for direct certification. |
