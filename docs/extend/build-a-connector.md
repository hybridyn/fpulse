...
This tutorial walks you end-to-end through building a working connector from scratch, four ways:
...
4. **[Derive from an existing Apache-2.0 OSS project](derive-from-talend.md)** — ~1 day, when an existing compatibly-licensed connector library already encodes years of production-tested vendor knowledge (auth flows, pagination quirks, error handling) for legacy enterprise systems and porting the logic is faster than rediscovering it from vendor docs

The first three produce a `v2.json` manifest from scratch and drop straight into the F-Pulse install. The fourth — derivation — is the right choice for legacy enterprise systems where production knowledge already exists in another Apache-2.0 codebase and porting the *logic* (not the code) into an F-Pulse manifest beats rediscovery. **No compile step, no IDE, no language tax** — we port the logic into the Python codebase rather than copying anything verbatim.
