...
For everything else, **the SQL Transform node usually solves it without a new node type** — try that first.

## Try the SQL Transform node first

Before writing any code, check whether the **SQL Transform** node already
covers it. It runs a DuckDB SQL statement over the input — joins, window
functions, CTEs, PIVOT, UNNEST, JSON path extraction, aggregates — and solves a
large share of "I need a custom transform" cases with no new node type and no
Python.

> **Note:** F-Pulse has **no built-in run-your-own-Python node** in either
> edition. If your logic genuinely can't be expressed in SQL, either write a
> first-class node type (below) or run the code outside F-Pulse and ingest the
> output via a CSV/JSON source.

## Write a first-class node type
