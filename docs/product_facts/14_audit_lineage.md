...
- **Sigstore signing** — every export tarball is signed via a Hybridyn Data Labs-issued sigstore certificate. Auditors can verify integrity without trusting the Hybridyn Data Labs API.
...
- ❌ Telling an OSS user "F-Pulse is SOC 2 certified" — the SOFTWARE isn't certified; the OPERATOR's deployment may bring F-Pulse into scope of the operator's own SOC 2. Plus customers get the additional Hybridyn Data Labs SOC 2 Type II attestation report on request.
