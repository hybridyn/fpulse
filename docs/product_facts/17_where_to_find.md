...
- **Doc**: `docs/product_facts/10_ai_copilot.md` enumerates all 26 tools by tier.
...
**Pipelines page → + New Pipeline** opens the template chooser. F-Pulse OSS ships **27 templates** covering the common starter shapes (simple ETL, dedup, aggregation, data quality, SCD2, API ingest with pagination, Slack alert on failure, etc.). Click any to start a pipeline pre-wired with that shape — you can edit it freely.
