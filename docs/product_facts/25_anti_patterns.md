...
get a Hybridyn SOC 2 Type II attestation report as supporting evidence."
...
**Why wrong:** The actual count is **37 OSS connectors** (4 database
dialects + 2 bulk-load dialects + ~31 SaaS manifests). Earlier marketing
claimed 60+ (corrected in May 3 batch to 45, then May 4 to 43) and the
2026-05-28 SSOT pass against `frontend/src/components/hiddenNodeTypes.ts`
landed on 37 — the SaaS manifest list was overcounted in the prior
audits.

**Correct:** "F-Pulse OSS ships 37 connectors: 4 database dialects
(postgresql, mysql, mssql, sqlite) + 2 bulk-load dialects (Postgres
COPY, Snowflake PUT+COPY) + ~31 SaaS manifests. The cert matrix
(`/api/connectors/cert-matrix`) shows live depth scores; today 8 are
v2 beta (validation in progress), 27 are v1 functional, and 2 are v1
basic."

**Source:** `edition-matrix.md` line 27.
...
**Correct:** "F-Pulse doesn't have a Notebook Node. For Python-based
transformations, F-Pulse+ offers the Python Transform node
(Docker-isolated). In OSS, use a DuckDB SQL Transform node, or run
your notebook externally and ingest the output via CSV/JSON Source."
...
Source/Sink, Python Transform) are Plus-only. The StepType enum has
88 entries but many are Plus-gated. The SSOT for the user-visible
palette is `frontend/src/components/hiddenNodeTypes.ts` (`VALID_GHOST_TYPES`).

**Correct:** "F-Pulse ships 40 node types across 6 categories in OSS.
Plus adds enterprise node types (JDBC, CDC, Vector, Python Transform).
See `02_node_types.md` for the per-node breakdown."
