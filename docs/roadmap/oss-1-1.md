...
capability gap surfaces from enterprise users, (c) the underlying
tech reaches the maturity needed for a clean implementation.
...
| Distribution = `pip install fpulse` — packaging + extras are done; publishing to PyPI is the remaining step. | Doesn't give the "double-click an app icon" feel. |
...
**Status:** specced internally; not implemented in 1.0.
