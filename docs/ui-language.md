...
3. **No false / aspirational capability claims.** If a detector, feature, or integration is contract-only / roadmap / not wired into live execution, label it as such. Active is "active"; planned is "planned". Carry a trust-marker naming the version something actually shipped in, so a reader can tell today's behaviour from the roadmap.
