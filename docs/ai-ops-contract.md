...
> F-Pulse AI is 26 tools the user can authorize an LLM to call on their behalf, with sanitization at the boundary, governance gates between the LLM and any side effect, and a daily-rebuilt retrieval layer that grounds answers in the user's own pipeline history.
