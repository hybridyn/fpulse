...
[![CI](https://github.com/hybridyn/fpulse/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/hybridyn/fpulse/actions/workflows/ci.yml)
[![Security Scan](https://github.com/hybridyn/fpulse/actions/workflows/security-scan.yml/badge.svg?branch=main)](https://github.com/hybridyn/fpulse/actions/workflows/security-scan.yml)
[![License: Apache 2.0](https://img.shields.io/badge/license-Apache%202.0-blue)](LICENSE)
[![Python 3.11+](https://img.shields.io/badge/python-3.11+-blue)](pyproject.toml)
[![Status: 1.0.0](https://img.shields.io/badge/status-1.0.0-blue)](CHANGELOG.md)
...
>
> Built and maintained by [Hybridyn](https://hybridyn.com).
...
- **Operational layer built in.** Scheduler (runs pipelines automatically at fixed times), alerts (email / Slack / Teams / webhook), run history with per-step row counts + duration, lineage view, multi-user workspaces with RBAC, version control with deploy / rollback. Ships in the box — nothing to bolt on.
...
- **Visual + code.** Drag-and-drop canvas, a real expression engine (`$json`, `$now`, `$('Node').output`), and Python / SQL transforms when you need to escape the canvas.
- **Open connector framework — extend in minutes, not weeks.** 33 first-party connectors visible by default (4 database dialects + 2 bulk-load dialects + 27 SaaS REST manifests), tier-labeled honestly (currently 0 Production + 0 Verified + 19 Beta + 8 Experimental). 10 additional consumer-marketing / SMB-CRM manifests ship Hidden — out of enterprise-data-engineering scope. See [docs/connectors.md](docs/connectors.md) for the per-connector matrix and live `GET /api/connectors/cert-matrix`. When you need one we don't ship, you have **four** first-class paths — none of them require a vendor build cycle:
  ...
  3. **Derive from an existing Apache-2.0 OSS project.** Any compatibly-licensed connector library can be a starting point — read the auth flow, pagination, and error handling, port the logic into an F-Pulse manifest, add a `NOTICE` entry. See [docs/extend/build-a-connector.md](docs/extend/build-a-connector.md) for the safe-derivation walkthrough.
...
| You are | Install path | Time |
|---|---|---|
| **A user on a single desktop** (no Python, no Docker) | Download the **packaged installer** below | 2 min |
| **An ops / platform team** | **Docker Compose** below | 5 min |
| **A developer / contributor** | **From source** below | 10 min |

### Desktop installer — Windows / macOS / Linux

The easiest path for non-developers. Bundles a frozen Python runtime
and the compiled UI — nothing else needs to be installed. After install
the app registers itself as an OS-native background service that auto-
starts at login and survives crashes / reboots.

| OS | Download | Install |
|---|---|---|
| Windows 10/11 (x64)  | `FPulse-Setup-1.0.0.exe` | Double-click → follow wizard |
| macOS 12+ (Intel + Apple Silicon) | `FPulse-1.0.0.pkg` | Double-click → follow wizard |
| Debian / Ubuntu 22.04+ | `fpulse_1.0.0_amd64.deb` | `sudo apt install ./fpulse_1.0.0_amd64.deb` |
| Fedora / RHEL 9+ | `fpulse-1.0.0.x86_64.rpm` | `sudo dnf install ./fpulse-1.0.0.x86_64.rpm` |
...
> **v1.0.0 release timing:** the prebuilt installers ship with the
> tagged v1.0.0 release. Until then, build any of them yourself in one
> command from this repo — see [`installer/readme.md`](installer/readme.md).
> Source / Docker installs work today.
...
Open <http://localhost:8001>, log in (the bootstrap password is in `data/INITIAL_ADMIN_PASSWORD.txt`), then open **Templates → First pipeline — CSV in, CSV out → Use Template → Run**. You'll see real output from a seeded sample dataset in under a minute — no Postgres, no API keys, no cloud account required.
...
> **Heads-up — `hybridyn/fpulse:1.0.0` builds locally on first run.** Until
> the v1.0.0 image is published to Docker Hub the compose / run commands
> above use the locally-built image (the `Dockerfile` at repo root). On
> first `docker compose up`, Docker auto-builds it from source — ~3-5
> min, one-time. No `pull access denied` errors despite the
> `hybridyn/fpulse` name.

### Single-container alternative (`docker run`)

If you don't want compose, one container is enough:
...
We welcome PRs, but every contributor must sign the
[Contributor License Agreement](CLA.md) — handled automatically via the
CLA-Assistant bot when you open your first PR.
