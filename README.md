...
|---|---|---|
| **A Linux desktop user** (no Python, no Docker) | **Linux package** below — `.deb` / `.rpm` / `.AppImage` | 2 min |
| **An ops / platform team** | **Docker Compose** below | 5 min |
| **A developer / contributor** | **From source** below | 10 min |

Two other paths exist but aren't published yet — we'd rather say so than
send you to a 404:

| Not yet available | Status |
|---|---|
| `pip install fpulse` | The package builds and runs; it is not on PyPI yet. Use **From source** below — same result, one extra command. |
| Windows `.exe` / macOS `.pkg` | Built and working, but **not shipped unsigned**: without an EV certificate Windows SmartScreen blocks the installer, and without Apple notarization macOS Gatekeeper refuses to open it. Shipping those would hand you a scary dialog with our name on it. Build either yourself in one command — see [`installer/readme.md`](installer/readme.md) — or use Docker / source. |
| `docker pull hybridyn/fpulse` | The image is not on Docker Hub yet, but **you don't need it** — `docker compose up` builds it locally on first run (see below). |
...
# 1. Build the UI. F-Pulse serves its own frontend, and the built bundle
#    is not committed — without this step the API runs but / has nothing
#    to serve. One-time; repeat only when frontend code changes.
