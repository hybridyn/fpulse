...
def _enable_self_registration() -> None:
    """Turn on self-service registration in admin_settings — F-Pulse OSS ships
    it OFF by default (single-operator), so register-flow tests must opt in the
    same way an operator would via Admin → Security."""
    import json
...
class TestE2EAuth:
    ...
    def test_full_auth_flow(self, client):
        # Self-registration is OFF by default (single-operator OSS); an admin
        # opts in via Admin → Security. Enable it so this flow can exercise
        # the register path.
