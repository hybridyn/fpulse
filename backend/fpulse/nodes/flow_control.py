...
class CodeScriptNode(BaseNode):
    ...
    Lock it down in untrusted / multi-tenant deployments with
    ``FPULSE_DISABLE_CODE_SCRIPT=1``.
    """
    ...
    def execute(self, ctx: ExecutionContext) -> duckdb.DuckDBPyRelation:
        ...
        # Kill-switch for untrusted / multi-tenant deployments. Code Script
        # runs user-provided Python in-process (restricted builtins + an
        # import allowlist, but NOT a sandbox), so a locked-down instance
        # can disable it entirely.
        import os as _os
        if _os.environ.get("FPULSE_DISABLE_CODE_SCRIPT", "").strip().lower() in {"1", "true", "yes", "on"}:
            raise ValueError(
                "Code Script is disabled on this instance "
                "(FPULSE_DISABLE_CODE_SCRIPT=1). It executes user-provided "
                "Python in-process and is not sandboxed."
            )
