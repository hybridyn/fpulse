...
class LocalTableSinkNode(BaseNode):
    ...
    def _mode_merge(
        ...
        # NULL-safe anti-join. The previous `(keys) NOT IN (SELECT keys ...)`
        # returns NULL (not TRUE) for EVERY existing row the moment the incoming
        # batch has a single NULL in a key column, so the WHERE dropped ALL
        # existing rows → silent total data loss on merge. NOT EXISTS with
        # IS NOT DISTINCT FROM compares keys NULL-safely and excludes only the
        # existing rows actually overwritten by an incoming key.
        key_match = " AND ".join(
            f'i."{c}" IS NOT DISTINCT FROM e."{c}"' for c in merge_on
        )
        merged = (
            f"SELECT e.* FROM _lt_existing e "
            f"WHERE NOT EXISTS (SELECT 1 FROM _lt_sink_input i WHERE {key_match}) "
