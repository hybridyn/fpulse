...
# SQL injection hardening. `schema` / `table` / `columns` / `watermark_column`
# are string-concatenated into SQL that runs against the real customer DB, and
# any of them can be expression-resolved from external-trigger `$vars` (the
# highest-priority var layer) before this node sees them. Identifiers cannot be
# parameterised, so whitelist them: a legitimate name matches a strict pattern,
# and anything carrying an injection payload (quotes, parens, spaces,
# semicolons, comment markers) is rejected up front. Quoting alone is NOT
# enough — an embedded double-quote escapes the quoted identifier.
_IDENT_RE = _re.compile(r"^[A-Za-z_][A-Za-z0-9_.]*$")


def _validate_identifier(name: str, *, kind: str = "identifier",
                         prefix: str = "DB Source") -> str:
    """Validate a SQL identifier (schema/table/column/watermark column).

    Accepts only ``^[A-Za-z_][A-Za-z0-9_.]*$`` and returns the validated name
    so call sites can inline it; rejects everything else with a user-safe error
    rather than letting an injected value reach the database.
    """
    s = (name or "").strip()
    if not s:
        raise ValueError(f"{prefix}: empty {kind} not allowed")
    if not _IDENT_RE.match(s):
        raise ValueError(
            f"{prefix}: invalid {kind} {name!r} — only letters, digits, "
            f"underscore and dot are allowed (got a value that could be a "
            f"SQL-injection payload). If this name is legitimate, alias it in "
            f"a query-mode SQL statement instead of table mode."
        )
    return s


def _reject_stacked_statements(clause: str, *, kind: str,
                               prefix: str = "DB Source") -> str:
    """Backstop for free-form clauses (``where`` / ``order_by``). These are
    intentionally operator-written SQL fragments, so they are not parsed — but
    they must never carry a stacked statement: a trailing ``;`` turns the
    clause into a second, attacker-chosen statement. External `$vars` must not
    be templated into these (operator-trusted only); rejecting ``;`` is the
    defence-in-depth backstop."""
    s = (clause or "").strip()
    if ";" in s:
        raise ValueError(
            f"{prefix}: {kind} clause must not contain ';' (stacked "
            f"statements are blocked). Put a single statement/expression here."
        )
    return s
...
class DbSourceNode(BaseNode):
    ...
    def execute(self, ctx: ExecutionContext) -> duckdb.DuckDBPyRelation:
        ...
        if watermark_col and watermark_val:
            ...
            # watermark_col is concatenated as an identifier and is
            # expression-resolvable from external `$vars`, so validate it
            # against the strict identifier pattern (quoting alone is not
            # enough). watermark_val stays single-quote-escaped.
            safe_col = _validate_identifier(watermark_col, kind="watermark column")
            safe_val = str(watermark_val).replace("'", "''")
            query = (
                f"SELECT * FROM ({query}) AS __incr "
                f'WHERE "{safe_col}" > \'{safe_val}\''
            )
    ...
    def _save_sync_cursor(self, ctx: "ExecutionContext", cursor_col: str,
        ...
        try:
            ...
            pending = getattr(ctx, "pending_sync_cursors", None)
            if pending is not None:
                # Deferred commit: buffer the new watermark; the executor
                # persists it at run-end ONLY if the whole run (incl. sinks)
                # succeeds. Advancing the cursor here — at source read-time —
                # would skip rows whose downstream sink then failed and never
                # actually landed (silent data loss). Empty for non-incremental.
                pending.append(state)
            else:
                # No run context to defer to (e.g. a direct unit test) —
                # persist immediately, preserving the historical behaviour.
                sync_state_store.upsert(state)
    ...
    def _build_table_query(self) -> str:
        ...
        # Validate every identifier that gets string-concatenated into the SQL;
        # a value carrying an injection payload is rejected before it runs.
        table = _validate_identifier(table, kind="table")
        if schema_name:
            schema_name = _validate_identifier(schema_name, kind="schema")
        if columns:
            col_names = [c.strip() for c in columns.split(",") if c.strip()]
            for c in col_names:
                _validate_identifier(c, kind="column")
        ...
        if where:
            # Operator-trusted free-form clause — reject stacked statements.
            sql += f" WHERE {_reject_stacked_statements(where, kind='WHERE')}"
        if order_by:
            sql += f" ORDER BY {_reject_stacked_statements(order_by, kind='ORDER BY')}"
        return sql
