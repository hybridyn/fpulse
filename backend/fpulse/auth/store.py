...
class UserStore:
    ...
    def _ensure_admin(self):
        """Self-heal an existing admin's role, and — ONLY when explicitly
        opted in via ``FPULSE_BOOTSTRAP_ADMIN=1`` — auto-seed a default admin
        when the user table is empty. By DEFAULT there is no auto-seed: the
        operator creates their own account on first launch (see below).
        ...
        Opt-in bootstrap (``FPULSE_BOOTSTRAP_ADMIN=1`` — for headless / Docker /
        CI deploys that can't do an interactive first run): generate a random
        24-char password, create ``admin@fpulse.local``, and write the password
        ONCE to ``INITIAL_ADMIN_PASSWORD.txt`` next to the SQLite db + log it.
        The operator reads it, signs in, rotates, deletes the file; we never
        read it back. WITHOUT the flag none of this happens — the DB stays empty
        and first-run registration (first_user_bootstrap) owns account creation.
        ...
        if not row:
            # No users yet. By DEFAULT F-Pulse does NOT auto-create an admin —
            # creating the account is the operator's decision. On first launch
            # the login page shows a "create account" step while the DB is empty
            # (see the first_user_bootstrap path); that first /register becomes
            # super_admin. This avoids shipping a known account + an on-disk
            # password file. Opt in to auto-seeding (random password written to
            # INITIAL_ADMIN_PASSWORD.txt) ONLY for headless / Docker / CI deploys
            # that can't do an interactive first run:
            #     FPULSE_BOOTSTRAP_ADMIN=1
            if os.environ.get("FPULSE_BOOTSTRAP_ADMIN", "").strip() == "1":
