...
REDACTION_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    # Connection-string / URL credentials: scheme://user:pass@host — the
    # password (and userinfo) is redacted. Runs FIRST so the ``@`` isn't
    # partially consumed by the email pattern. Covers postgres://, mysql://,
    # mongodb://, redis://, amqp://, https:// basic-auth, etc. The 32+ char
    # api_key heuristic below never matched these because of the ://:@ chars.
    ("conn_credential", re.compile(r"(?i)\b[a-z][a-z0-9+.\-]*://[^\s:/@]+:[^\s/@]+@")),
    # key=value / key: value secrets in free text (password=..., secret: ...,
    # token=..., short secrets the api_key length heuristic misses).
    ("kv_secret", re.compile(
        r"(?i)\b(?:password|passwd|pwd|secret|token|api[_-]?key|client_secret|"
        r"access_key|secret_key|connection_string|sas_token)\b\s*[:=]\s*\S+"
    )),
    # Email
