...
class Database:
    ...
    def _migrate_v3_signup_default(self, conn: sqlite3.Connection) -> None:
        """v2 → v3: historically flipped `allow_self_registration` False → True
        to roll open self-signup out as the default.

        REVERSED 2026-07-20 — NO-OP now. F-Pulse OSS is a single-operator
        install and ships self-registration OFF by default (see
        api/auth._read_signup_policy). Force-enabling signup contradicts that
        and — as this migration's own history noted — did it "without admin
        consent", which on a network-exposed instance let strangers create
        accounts. This migration therefore no longer touches
        `allow_self_registration`: a fresh install keeps the signup-off default,
        and an admin's explicit choice (either way) always sticks. The
        idempotency marker is still written so schema bookkeeping stays stable
        and this never resurrects the old flip on a later run.
        """
        conn.execute(
            "INSERT OR REPLACE INTO _meta (key, value) VALUES (?, ?)",
            ("signup_default_v3", "1"),
    ...
    def _migrate_v18_composite_indexes(self, conn: sqlite3.Connection) -> None:
        ...
        # (index_name, table, ddl). The explicit table lets us skip cleanly when
        # a table isn't present in *this* edition — `audit_log` is Plus-only
        # (OSS writes audit events to the structured logger, not a table; see
        # fpulse/audit.py), so a fresh OSS install must not log a scary
        # "no such table" warning here. The warning path is kept for GENUINE
        # failures (table exists but is missing an expected column).
        indexes = [
            # Executions: "recent runs for workflow X" (admin + history pages).
            ("idx_executions_wf_started", "executions",
            ...
            ("idx_executions_ws_started", "executions",
             "CREATE INDEX IF NOT EXISTS idx_executions_ws_started "
             "ON executions(workspace_id, started_at DESC)"),
            # Audit log (Plus-only table): "what did user X do recently".
            ("idx_audit_user_time", "audit_log",
             "CREATE INDEX IF NOT EXISTS idx_audit_user_time "
             "ON audit_log(user_id, timestamp DESC)"),
            # Audit log (Plus-only table): "recent actions of type X".
            ("idx_audit_action_time", "audit_log",
             "CREATE INDEX IF NOT EXISTS idx_audit_action_time "
             "ON audit_log(action, timestamp DESC)"),
            # Alert logs: "recent alerts for workflow X" (monitor page).
            ("idx_alert_logs_wf_triggered", "alert_logs",
            ...
            ("idx_alert_logs_ws_triggered", "alert_logs",
            ...
            ("idx_lifecycle_wf_time", "lifecycle_events",
            ...
            ("idx_lifecycle_ws_time", "lifecycle_events",
        ...
        for name, table, ddl in indexes:
            if not _table_exists(table):
                # Expected in some editions (e.g. audit_log on OSS). Not an error.
                skipped += 1
                logger.debug(
                    "F-Pulse schema v18: skipping index %s — table %s is not "
                    "present in this edition.", name, table,
                )
                continue
            try:
                conn.execute(ddl)
        ...
        logger.info(
            "F-Pulse: schema v18 — %d/%d composite indexes created (%d skipped: "
            "table absent in this edition)",
            created, len(indexes), skipped,
        )
