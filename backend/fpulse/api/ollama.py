...
def _ollama_url(request: Request | None = None) -> str:
    ...
    final = _normalize_localhost(resolved).rstrip("/")
    # SSRF egress guard — a caller-supplied ollama base_url pointing at a
    # private / metadata / internal host (NOT loopback, which is Ollama's home)
    # is refused for non-admins, so /status /pull /delete can't be turned into
    # an internal-network probe. Admins may target internal hosts.
    from fpulse.security.ssrf import is_internal_host
    if is_internal_host(final):
        _u = current_user_optional(request) if request is not None else None
        if getattr(_u, "role", None) not in ("super_admin", "admin"):
            raise HTTPException(
                403,
                "The configured Ollama base_url points at a private/internal "
                "host; this is restricted to admins.",
            )
    return final
