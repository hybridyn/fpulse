...
def _read_signup_policy() -> dict:
    ...
    from fpulse.main import app_state
    db = app_state.get("db")
    # Default = True. F-Pulse OSS ships open-signup by default so individual
    # engineers can install on a laptop, sign up for their own account, and
    # immediately get a personal workspace. Admins who want invite-only mode
    # explicitly toggle it off in Admin → Security; the workspace-level
    # `domain_allowlist` still gates which users can be added to a corporate
    # workspace, so flipping this on does NOT relax corporate scoping.
    allow = True
    if db:
        import json
        row = db.fetchone("SELECT data FROM settings WHERE id = 'admin_settings'")
        if row:
            try:
                allow = bool(json.loads(row["data"]).get("allow_self_registration", True))
            except Exception:
                allow = True
    ...
    # just installed F-Pulse and shipped it with `allow_self_registration=False`
    # (the new default) would have no way to create the very first account.
...
async def register(body: RegisterRequest):
    ...
    # OSS: every account is a developer. Single-user installs don't need
    # multi-tier roles; the seeded operator and any self-registered users
    # all get developer-level access.
