...
_DOC_CATALOG: list[dict] = [
    ...
    # (May 6 2026, user-reported redundancy).
    {
        ...
        "audience": "All users",
        "summary": "43 connectors with depth scores. Certified, Beta, F-Pulse+ enterprise.",
        "admin_only": False,
        "plus_only": False,
    },
    {
    ...
    },
    {
        ...
        "audience": "Developers",
        "summary": "5-minute Python transform path (no registration needed) or 30-minute first-class node-type path (subclass + StepType enum + registered). When to pick which.",
        "admin_only": False,
        "plus_only": False,
    },
    {
    ...
    # 2026-05-29: derivation infrastructure for closing the
    # connector-gap via porting from compatibly-licensed Apache-2.0
    # source. The process doc + roadmap make the path safe + trackable
    # instead of a one-off "is this legal" question on every port.
    # The doc titles below describe the comparison/derivation artefact —
    # readers click them precisely because they want to read about
    # that comparison. Summaries stay generic so a casual Help-page
    # browser sees neutral copy in the sidebar.
    {
        "path": "extend/derive-from-talend.md",
        "title": "Derive a connector from upstream Apache-2.0 source",
        "audience": "Developers",
        "summary": "Safe, repeatable workflow for porting connector logic from any compatibly-licensed OSS source into an F-Pulse manifest. License-verification checklist, attribution template, what to port vs what to skip, common pitfalls.",
        "admin_only": False,
        "plus_only": False,
    },
    {
        "path": "extend/talend-derivation-roadmap.md",
        "title": "Connector derivation roadmap",
        "audience": "Maintainers",
        "summary": "Prioritised list of legacy enterprise connectors worth porting from compatibly-licensed upstream OSS sources. Top 5, next 10, opportunistic tier. Effort estimates + source paths + strategic rationale.",
        "admin_only": False,
        "plus_only": False,
    },
    ...
    # NOTE: execution-architecture.md was previously catalogued here but
    # withdrawn (May 9 2026) — the doc reads as a Plus-tier enterprise
    # workload pitch (stale $100/mo pricing, claims process-per-pipeline
    # isolation that's the not-yet-shipped F-Pulse+ Stage 5 design, lists
    # RBAC + audit trail + PROD env as included). Surfacing it on the OSS
    # Help page misled users about what OSS Free actually does. Re-add
    # only after a full rewrite that matches the OSS thread-pool model.
    {
        "path": "testing.md",
        "title": "Testing guide",
        "audience": "Developers",
