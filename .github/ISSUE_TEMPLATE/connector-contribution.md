...
- [ ] Hand-written Python node (subclassed `TransformNode` / `SourceNode` / ...)
...
(Required for compliance — gets copied into the repo-root `NOTICE` file by the reviewer when the PR merges. See [docs/extend/derive-from-talend.md](https://github.com/hybridyn/fpulse/blob/main/docs/extend/derive-from-talend.md) for the full safe-derivation process — applies to any Apache-2.0 upstream, not only the project named in the filename.)
