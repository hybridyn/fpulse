...
# to /usr/lib/fpulse/frontend-dist, which main.py never looks for — the
# package installed cleanly and served a blank page. The UI now travels
# inside the PyInstaller bundle via the staging step above.

# Verify the frozen bundle actually carries the UI, rather than discovering it
# after the .deb is on someone's machine.
if ! find "$STAGING/usr/lib/$PKG_NAME/runtime" -path '*/fpulse/frontend_dist/index.html' -print -quit | grep -q .; then
