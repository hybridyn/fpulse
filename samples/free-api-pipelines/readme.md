...
│   ├── 17-event-driven-alerts.json         ← 2026-05-26 (SQL Transform + Slack/Email)
...
| **17** | **GitHub `/repos/duckdb/duckdb/issues`** | filter 24h → **`transform`** (SQL severity classifier) → **`if_condition`** → **`slack_notify`** + main branch → aggregate → **`send_email`** digest + parallel **`set_variable`** → **`http_request`** monitoring metric | (no sink — notifications only) | logs |
...
- **ADDED v3 (2026-05-26)** #14 (`lookup` + `scd2`), #15 (`pivot` + `unpivot`), #16 (`validate` + `conditional_split` + `sample`), #17 (`if_condition` + `slack_notify` + `send_email` + `set_variable` + `http_request`), #18 (`xml_source` + `execute_sql_task` + `validation` + `foreach_loop` + `file_system`).
...
| **17** — **Event-Driven Alerts** | GitHub issues filtered to last 24h, classified by a SQL `transform` (CASE on title keywords) into critical/major/minor; `if_condition` routes critical rows to Slack; main flow aggregates by label and emails a digest; a parallel branch sets a counter variable and POSTs a JSON metric payload to a monitoring webhook. **Slack / SMTP / webhook URLs are PLACEHOLDERS** — see the pipeline's top-level description. | `if_condition`, `slack_notify`, `send_email`, `set_variable`, `http_request` |
...
- **XML source DOES exist** (`xml_source`) but reads from a local file path, not a URL — so a "World Bank XML" demo would need a shipped sample XML file. Skipped to keep the sample set portable. Easy to add if you ship a small sample XML file alongside it.
